...
public class UserDaoImpl implements UserDao {
	...
	public List<User> getUserList() {
		...
		try {
			transaction = session.beginTransaction();
			List uq = session.createQuery("from User where  isValid='1'")
					.list();
	...
	private boolean updateUser(String name, String password, Integer type,
			String email) {
	...
	public boolean updateUserType(String name, Integer type) {
		...
		if (this.isUserValid(name) == false) {
			logger.error("User: " + name
					+ ", is invalid now, forbidden to update the status");
			return false;
		}
		Session session = HibernateUtil.getSessionFactory().openSession();
		Transaction transaction = null;
	...
	public boolean authUser(String name, String password) {
		...
		try {
			transaction = session.beginTransaction();
			List ul = session.createQuery(
					"from User where name='" + name + "' and password='"
							+ password + "'").list();
			if (ul.size() == 1) {
				authStatus = true;
			}
	...
	private boolean isUserExisted(String name) {
		...
		try {
			transaction = session.beginTransaction();
			List ul = session
					.createQuery("from User where name='" + name + "'").list();
			if (ul.size() == 1)
	...
	public Integer getUserIdByUsername(String name) {
		...
		try {
			...
			if (ul.size() != 1) {
				logger.error("There should be just one user existed with name: "
						+ name + ", but now there are: " + ul.size());
				return null;
			}
			...
			List ul = session
					.createQuery(
							"from User where name='"
									+ name
									+ "'  and type in (select id from Usertype where name ='"
									+ UserEnumType.admin + "')").list();
	...
	public boolean isEmailValid(String uEmail) {
		...
		try {
			transaction = session.beginTransaction();
			List ul = session.createQuery(
					"from User where email='" + uEmail + "'").list();

			if (ul.size() != 1) {
				logger.error("There should be just one user existed with email: "
						+ uEmail + ", but now there are: " + ul.size());
				return false;
			}
			for (Iterator iterator = ul.iterator(); iterator.hasNext();) {
				User u = (User) iterator.next();
	...
	public User getUserByUserId(Integer userId) {
		...
		try {
			transaction = session.beginTransaction();
			List ul = session
					.createQuery("from User where id='" + userId + "'").list();

			if (ul.size() != 1) {
				logger.error("There should be just one user existed with userId: "
						+ userId + ", but now there are: " + ul.size());
				return null;
			}
