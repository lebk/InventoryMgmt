...
public class JobServiceImpl implements JobService {
	 ...
	 * product and os.f
	 */
	public Integer startJob(Integer poId, Integer osId, Boolean isProdSetting,
			Boolean isProdActivate, Integer userId) {
		if (poId == null || osId == null || userId == null) {
			logger.error("the passsed in poId:" + poId + "or osId:" + osId
					+ "OR userId:" + userId + " is null!");
			return null;
		}
		...
		if (this.isVMCreating(osId, poId) || this.isVMExisted(osId, poId)) {
			logger.warn("there are VM is in creating status or existed with osId="
					+ osId + ", poId=" + poId);
			return null;
		}
		...
		{
			logger.error("the product id: " + poId
					+ " is not able to install in os Id:" + osId);
			return null;
		}
		return this.startJob(poId, osId, isProdSetting, isProdActivate, false,
				userId);
	}
	 ...
	 */
	public Integer startJob(Integer poId, Integer osId, Boolean isProdSetting,
			Boolean isProdActivate, boolean override, Integer userId) {
		if (us.isUserAdmin(userId) == true) {
			// admin user

			if (override == false) {
				if (this.isVMCreating(osId, poId)
						|| this.isVMExisted(osId, poId)) {
					logger.warn("There are VM with the poId=" + poId
							+ ", osId=" + osId
							+ "existed or creating, return null");
					return null;
				}
				return this.run(osId, poId, isProdSetting, isProdActivate,
						userId);
			...
			if (override == true) {
				...
				if (this.isVMCreating(osId, poId)
						|| this.isVMExisted(osId, poId)) {
					logger.warn("some image is already eixsted or creating with osId="
							+ osId + ", poId=" + poId);
					return null;
				} else {
	...
	/*
	 * Stop a job, only admin user or the user who create the job can stop a
	 * job.
	 */
	 ...
	 * admin, then all the jobs will returned, otherwise, only the owned job
	 * will be returned.
	 */
	public List<Job> getJobsByUser(Integer userId, Integer pageSize,
			Integer pageNow) {
	...
	public boolean isAllowedToStop(Integer jobId, Integer userId) {
		...
		if (us.isUserAdmin(userId) || this.isJobOwner(jobId, userId)) {
			// Only the following status and the job is not request to stop
			// before can
			// allow to stop
			if (JobStatusEnumType.getValidStatusToBeStop().contains(
					this.getCurrentJobStatus(jobId, userId))
					&& this.isJobRequestToStop(jobId) == false) {
				return true;
			}
		}
		return false;
	...
	private Integer run(Integer osId, Integer poId, boolean isSettingProduct,
			Boolean isProdActivate, Integer userId) {
		...
		Integer jobId = jd.startJob(osId, poId, isSettingProduct,
				isProdActivate, userId);
	...
	private boolean stop(Integer jobId, Integer userId) {
		// For stop job, we just mark the job's status (isRequestToStop to "Y",
		// when
		// the
	...
	public Job getJobByOsIdAndPoId(Integer osId, Integer poId) {
		if (osId == null | poId == null) {
			logger.error("osId or poId could not be null: osId: " + osId
					+ ", poId=" + poId);
		}
		...
		for (Job j : jl) {
			String jobStatus = JobStatusEnumType.getJobStatusbyStatusId(j
					.getJobStatusId());
			logger.info("job, id: " + j.getId() + ", the job status is: "
					+ jobStatus);

			if (JobStatusEnumType.getRunningStatusList().contains(jobStatus)
					|| JobStatusEnumType.complete.equals(jobStatus)) {
				logger.info("Gotta one running/complete job!job, id: "
						+ j.getId() + ", the job status is: " + jobStatus);
