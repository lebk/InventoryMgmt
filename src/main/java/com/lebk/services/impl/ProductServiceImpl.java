...
public class ProductServiceImpl implements ProductService {
	...
	public boolean updateProduct(String pName, String ptType, String ptColor,
		...
		return pd.updateProduct(pName, ptTypeId, ptColorId, ptSizeId, pNum,
				btId, opUserId);
	}
	...
	public boolean cleanUpAll(String opUser) {
		...
		if (!UserEnumType.getUsertypeById(userTypeId)
				.equals(UserEnumType.admin)) {
	...
	private String constructProductName(String ptType, String ptColor,
			String ptSize) {
	...
	public String getProductNumber(String ptType, String ptColor, String ptSize) {
		if ((ptType == null || ptType.length() == 0)
				|| (ptColor == null || ptColor.length() == 0)
				|| (ptSize == null || ptSize.length() == 0)) {
	...
	public List<Product> searchProduct(String ptType, String ptColor,
		...
		Integer ptTypeId = null;
		if (ptType == null || ptType.length() == 0) {
			ptTypeId = null;
		} else {
			ptTypeId = pts.getIdByPtType(ptType);
		}

		Integer ptColorId = null;
		if (ptColor == null || ptColor.length() == 0) {
			ptColorId = null;
		} else {
			ptColorId = pcs.getIdByPtColor(ptColor);
		}

		Integer ptSizeId = null;
		if (ptSize == null || ptSize.length() == 0) {
			ptSizeId = null;
		} else {
			ptSizeId = pss.getIdByPtSize(ptSize);
		}

		return pd.searchProduct(ptTypeId, ptColorId, ptSizeId);
	}
}
