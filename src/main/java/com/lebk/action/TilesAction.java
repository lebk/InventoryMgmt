...
public class TilesAction extends ActionSupport {
	...
	public String showProductDetailsList() {
		...
		this.productDetailsList = ProductUtil
				.convertPtdetailsListToPtdetailsDTOList(pdl);
	...
	public String deleteProductType() {
		...
		logger.info("The selected product type id is:"
		...
		if (status == true) {
			logger.info("Delete the product type successfully, the product type id is:"
	...
	public String deleteProductColor() {
		...
		logger.info("The selected product color id is:"
				+ this.selectedProductColorId);
		logger.info("The user is::" + userName);

		Boolean status = pcs.deletePtColor(this.selectedProductColorId,
				userName);

		if (status == true) {
			logger.info("Delete the product color successfully, the product color id is:"
	...
	public String deleteProductSize() {
		...
		logger.info("The selected product size id is:"
				+ this.selectedProductSizeId);
		logger.info("The user is::" + userName);

		Boolean status = pss.deletePtSize(this.selectedProductSizeId, userName);

		if (status == true) {
			logger.info("Delete the product size successfully, the product size id is:"
	...
	public String addUser() {
		...
		if (true == addasAdmin) {
			boolean badd = us.addUser(addUserName, addUserPassword, iAdminType,
					opUser);
			if (true == badd)
				return SUCCESS;
			else
			...
			boolean badd = us.addUser(addUserName, addUserPassword,
					iRegularType, opUser);
			if (true == badd)
				return SUCCESS;
			else
	...
	public String deleteUser() {
		...
		if (us.getUserByUserId(this.selectedUserId).getName()
	...
	public String updateUser() {
		...
		if (true == bAdmin) {
			boolean bUpdate = updateUserType(selectedUserId, iRegularType,
					userName);
			if (true == bUpdate) {
				return SUCCESS;
			} else
			...
			boolean bUpdate = updateUserType(selectedUserId, iAdminType,
					userName);
			if (true == bUpdate) {
				return SUCCESS;
			} else
	...
	private void convertUserListToUserDTOList(List<User> ul,
	...
	private void convertProductListToProductDTOList(List<Product> pl,
