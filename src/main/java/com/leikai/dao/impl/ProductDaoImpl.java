...
public class ProductDaoImpl implements ProductDao {
	...
	public boolean addProduct(String pName, String version, String key,
			String baseLocation, String prodType, List<String> supportedOsList,
			String uploadUser) {
	...
	public Integer getIdByProdName(String pName) {
		{
			...
			try {
				...
				List pl = session.createQuery(
						"from Product where name='" + pName + "'").list();
				if (pl.size() == 0) {
					logger.warn("no product found by the name: " + pName);
					return null;
				}
				if (pl.size() > 1) {
					logger.error("more than one product found by the name: "
							+ pName);
					return null;
				}
	...
	private Integer getprodTypeIdByType(String prodType) {
		...
		try {
			transaction = session.beginTransaction();
			List ptl = session.createQuery(
					"from Producttype where name='" + prodType + "'").list();
			if (ptl.size() != 1) {
			...
			for (Iterator it = ptl.iterator(); it.hasNext();) {
				...
				logger.info("ProductType: " + prodType + ", id is: "
						+ pt.getId());
	...
	public String getProdTypebyProdTypeId(Integer prodTypeId) {
		...
		try {
			transaction = session.beginTransaction();
			List ptl = session.createQuery(
					"from Producttype where id='" + prodTypeId + "'").list();
			if (ptl.size() != 1) {
			...
			for (Iterator it = ptl.iterator(); it.hasNext();) {
				Producttype pt = (Producttype) it.next();
				logger.info("ProductType: " + prodTypeId + ", name is: "
						+ pt.getName());
	...
	public String getNameByProductId(Integer poId) {
		{
			...
			try {
				transaction = session.beginTransaction();
				List pl = session.createQuery(
						"from Product where id='" + poId + "'").list();

				if (pl.size() > 1) {
					logger.error("There should be just one product existed with product id: "
							+ poId + ", but now there is: " + pl.size());
	...
	public boolean updateProductName(String oldName, String newName) {
		...
		try {
			transaction = session.beginTransaction();
			List pl = session.createQuery(
					"from Product where name='" + oldName + "'").list();
			if (pl.size() > 1) {
				logger.error("There should be just one product existed with product name: "
						+ oldName + ", but now there is: " + pl.size());
			...
			for (Iterator iterator = pl.iterator(); iterator.hasNext();) {
				...
				pro.setName(newName);
				String hql = "update Product set name='" + newName
						+ "'where name='" + oldName + "'";
